...
public:
  Ising1d(int nspins, double hfield, bool pbc = true)
      : nspins_(nspins), hfield_(hfield), pbc_(pbc) {
    Init();
  }
  ...
  void Init() {
    ...
    std::cout << "# Using the 1d Transverse-field Ising model with h = "
              << hfield_ << std::endl;
  }
  ...
  void FindConn(const std::vector<int> &state,
                std::vector<std::vector<int>> &flipsh,
    ...
    // iterate through all the spins, adding the -Z_i Z_j terms
    for (int i = 0; i < (nspins_ - 1); i++) {
      mel[0] -= double(state[i] * state[i + 1]);
    }

    // if `pbc_` was set as true, then the last and first spins are
    // considered to be connected with each other (in practice, an
    // additional -Z_1 Z_N energy contribution is added)
    if (pbc_) {
